...
PID balancePid(900,5,50,0);//(9000, 5, 30, 0); //(9000, 17, 80, 0); //adjust
PID speedPid(3, 0, 1, 0); // (1.0, 0.38, 0.23, 0); //adjust 0.38 1 以及 下面的2个
...
void loop() {
  ...
  if (millis() > loopTimer) {
    ...
    if (cmd == 'w') {
      ...
      vDesired = 30; // stop speed command
    ...
    double targetPitch = speedOutput * 0.00038; 
    ...
    Serial.println(speedCmPerSecond);
      ...
      if (balanceOutput > 0) { // for pitch direction check, not a must
      step1.setTargetSpeedRad (-30); //can never reach, just to make it move or proportional to vDesired
      step2.setTargetSpeedRad(30);
    } else {
      step1.setTargetSpeedRad(30);
      step2.setTargetSpeedRad(-30);
